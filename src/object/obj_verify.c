		  ...
		  uint32_t reps, uint32_t shard_a, uint32_t shard_b)
{
	...
	if (!daos_key_match(&cur_a->dkey, &cur_b->dkey)) {
		D_INFO(DF_OID" (reps %u, inconsistent) "
		       ...
		       (char *)cur_a->dkey.iov_buf, shard_b,
		       (char *)cur_b->dkey.iov_buf);
		return -DER_MISMATCH;
	}
	...
	if (!daos_key_match(&cur_a->iod.iod_name, &cur_b->iod.iod_name)) {
		D_INFO(DF_OID" (reps %u, inconsistent) "
		       ...
		       DP_OID(oid), reps, shard_a,
		       (char *)cur_a->iod.iod_name.iov_buf, shard_b,
		       (char *)cur_b->iod.iod_name.iov_buf);
		return -DER_MISMATCH;
	}
	...
	if (cur_a->iod.iod_recxs->rx_idx != cur_b->iod.iod_recxs->rx_idx) {
		D_INFO(DF_OID" (reps %u, inconsistent) "
	...
	if (cur_a->iod.iod_recxs->rx_nr != cur_b->iod.iod_recxs->rx_nr) {
		D_INFO(DF_OID" (reps %u, inconsistent) "
		       "shard %u has EV rec len %lu, "
		       "but shard %u has EV rec len %lu.\n",
		       DP_OID(oid), reps, shard_a, cur_a->iod.iod_recxs->rx_nr,
		       shard_b, cur_b->iod.iod_recxs->rx_nr);
		return -DER_MISMATCH;
	}

	if (cur_a->type == OBJ_ITER_SINGLE)
		return 0;

	D_ASSERT(cur_a->type == OBJ_ITER_RECX);
